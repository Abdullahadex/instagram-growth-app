...
function requiredEnv(vars: string[]) {
...
app.get('/auth/facebook/login', (_req, res) => {
	...
	const redirect = process.env.FB_REDIRECT_URI as string;
	const scopes = [
		'pages_show_list',
...
app.get('/auth/facebook/callback', async (req, res) => {
	try {
		...
		if (!shortResp.ok) return res.status(500).json(shortData);

		const exchangeUrl = buildUrl('oauth/access_token', {
			grant_type: 'fb_exchange_token',
			client_id: process.env.FB_APP_ID!,
			client_secret: process.env.FB_APP_SECRET!,
			fb_exchange_token: (shortData as any).access_token,
		});
		const longResp = await fetch(exchangeUrl);
		const longData = await longResp.json();
		if (!longResp.ok) return res.status(500).json(longData);

		res.json({
			message: 'Authenticated. Save this long-lived user token server-side.',
			long_lived_user_token: (longData as any).access_token,
			expires_in: (longData as any).expires_in,
		});
	} catch (err: any) {
